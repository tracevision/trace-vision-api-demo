...
def resize_video(input_path, output_path, target_width, target_height, target_fps=12):
    ...
    if target_fps is not None:
        ...
        print(
            f"Resizing video to {target_width}x{target_height} and changing FPS to {target_fps}..."
        )
    else:
...
def main():
    ...
    ap.add_argument(
        "--input_path", required=True, help="The path to the input video file."
    )
    ap.add_argument(
        "--output_path", required=True, help="The path to the output video file."
    )
    args = ap.parse_args()
    ...
    output_path = input_path.replace(
        "60_min_chunks_right", "60_min_12fps_resized_right"
    )
